...
const state = {
  notifications: [
    ...
    {
      text: "Beliebter Beitrag: '#CATURDAY! BILDER VON EUREN HAUSTIEREN HIER HER!'",
      read: false,
...
const mutations = {
  [MARK_AS_READ]: ({ notifications }, index) => notifications[index].read = true,
  [MARK_AS_UNREAD]: ({ notifications }, index) => notifications[index].read = false,
  [MARK_ALL_AS_READ]: ({ notifications }) => notifications.forEach(n => n.read = true)
};
